...
class crv_inputs(crv.Randomized):
    def __init__(self,bin):
        ...
        self.bin = bin
        self.add_rand("bin",list(range(2**g_bin_width)))
...
class RandomSeq(uvm_sequence):
    async def body(self):
        while(len(covered_values) != 2**g_bin_width):
            data_tr = SeqItem("data_tr", None)
            await self.start_item(data_tr)
            data_tr.randomize_operands()
            while(data_tr.i_crv.bin in covered_values):
                data_tr.randomize_operands()
            covered_values.append(data_tr.i_crv.bin)
...
class Driver(uvm_driver):
    ...
    async def run_phase(self):
        ...
        while True:
            ...
            await self.bfm.send_data(data.i_crv.bin)
    ...
    def check_phase(self):
        ...
        while self.result_get_port.can_get():
            ...
            if not data_success:
                ...
                bin = data
                if bin == bcd_to_dec(actual_result):
                    # pass
...
@pyuvm.test()
class Test(uvm_test):
    """Test Bin 2 Bcd with random values"""
